...
import { defaultAiAvailability, parseAvailabilityResponse, type AiAvailability, type AiCapability, type AiProviderId } from "../lib/ai-availability";
...
export function AiModelControl({ capability, value, onChange }: { capability: AiCapability; value: AiProviderId; onChange: (provider: AiProviderId, model: string, available: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!active) return;
      if (!error) setRows(parseAvailabilityResponse(data));
      setLoading(false);
    })();
    return () => { active = false; };
  }, [capability]);
