...
function schemaFor(capability: string): Record<string, unknown> {
  ...
  if (capability === "vocabulary-card") return { type: "object", additionalProperties: false, required: ["chineseMeaning", "englishDefinition", "pronunciation", "example"], properties: { chineseMeaning: { type: "string" }, englishDefinition: { type: "string" }, pronunciation: { type: "string" }, example: { type: "string" } } };
...
function instructionsFor(capability: string): string {
  ...
  if (capability === "vocabulary-card") return "Create one compact learner vocabulary card for the exact selected word or phrase. Return concise Simplified Chinese meaning, learner-friendly English explanation, useful pronunciation notation, and one natural example sentence matching the supplied context.";
